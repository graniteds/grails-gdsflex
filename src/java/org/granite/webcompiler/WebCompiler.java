...
public class WebCompiler {
	...
	protected void configure(Configuration configuration, WebCompilerType type) throws WebCompilerException {
		...
		configuration.addSourcePath(new File[]{new File(this.basePath)});
		configuration.addLibraryPath(internalFiles.toArray(new File[internalFiles.size()]));
		configuration.addExternalLibraryPath(externalFiles.toArray(new File[externalFiles.size()]));
